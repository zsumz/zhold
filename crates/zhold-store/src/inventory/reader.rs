...
use zhold_core::{ArenaId, ArenaRecord, ArenaState, ByteSize};
...
fn read_entry(
    ...
    let name = arena_path
        ...
        .ok_or_else(|| StoreError::NonUnicode {
            kind: "arena directory name",
            path: arena_path.to_path_buf(),
        })?;
    let arena_id = ArenaId::from_str(name).map_err(|error| StoreError::InvalidOwnership {
        path: arena_path.to_path_buf(),
        reason: error.to_string(),
    })?;
    if layout.arena(&arena_id).as_path() != arena_path {
        return Err(StoreError::InvalidOwnership {
            path: arena_path.to_path_buf(),
            reason: "arena path does not match the indexed identity".to_owned(),
        });
    }
    ...
    let size = measure_tree(arena_path)?;
    let active = matches!(
